/*
   1- use from tag and onSubmit handler with event (e) from
   e access e.target.[name of the input field].value.

   2 -  Form with state to get value and set default value: 
      controlled element: use individual field state for each input field value

   3 - controlled element: one object  in a
   state holding all the input field value

   4-  uncontrolled element: useRef to create a reference to the element and access value by using like: nameRef.current.value

   5- use hook to handle state and submit and error. example react hook form.

   6- reuseable components

*/